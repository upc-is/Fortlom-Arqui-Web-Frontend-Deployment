...
class AuthStateManager {
  ...
  updateAuthState() {
    if (!this._sdk.emitter.e || !this._sdk.emitter.e[EVENT_AUTH_STATE_CHANGE] || !this._sdk.emitter.e[EVENT_AUTH_STATE_CHANGE].length) {
      (0, _util.warn)('updateAuthState is an asynchronous method with no return, ' + 'please subscribe to the latest authState update with ' + 'authStateManager.subscribe(handler) method before calling updateAuthState.');
    }
    ...
    const {
      autoRenew,
      autoRemove
    } = this._sdk.tokenManager._getOptions();
    ...
    const cancelablePromise = new PCancelable((resolve, _, onCancel) => {
      ...
      this._sdk.tokenManager.getTokens().then(({
        ...
        if (accessToken && this._sdk.tokenManager.hasExpired(accessToken)) {
          isPending = shouldEvaluateIsPending(); // if autoRenew is pending, continue to use the expired token while renew is in process
